...
class TANK:
    ...
    def reset_dataframe(self,days_back):
        cur_time = dt.datetime.now()
        old_data = self.history_df[self.history_df.datetime<(cur_time-dt.timedelta(days=days_back))]
        if len(old_data)>0:
            ...
            self.history_df = self.history_df[self.history_df.datetime>=(cur_time-dt.timedelta(days=days_back))]  
