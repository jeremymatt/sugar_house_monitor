...
LOGROTATE_DST="/etc/logrotate.d/sugar-display"
...
LOG_PATH="${USER_HOME}/display_controller.log"
...
if [[ "$1" == "-on" ]]; then
  ...
  if [[ -f "${LOGROTATE_SRC_DIR}/display_controller" ]]; then
    render_unit "${LOGROTATE_SRC_DIR}/display_controller" "${LOGROTATE_DST}"
  fi
  systemctl daemon-reload
  systemctl enable --now sugar-display.service
  echo "Enabled sugar-display.service (logs -> ${LOG_PATH})"
else
  systemctl disable --now sugar-display.service || true
  systemctl stop sugar-display.service || true
  systemctl reset-failed sugar-display.service || true
  echo "Disabled sugar-display.service"
fi
